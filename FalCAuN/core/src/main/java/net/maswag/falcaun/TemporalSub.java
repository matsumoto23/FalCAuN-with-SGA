...
public class TemporalSub<I> extends AbstractTemporalLogic<I> {
    ...
    public String toOwlString() {
        if (subFml instanceof TemporalEventually) {
            ...
            else if (from == 0) { return "(" + ((TemporalEventually<I>)subFml).getSubFml().toOwlString() + ") | (" +  new TemporalNext<I>(new TemporalSub<I>(subFml, from, to - 1), true).toOwlString() + ")"; }
            else { return new TemporalNext<I>(new TemporalSub<I>(subFml, from - 1, to - 1), true).toOwlString(); }
        } else {
            if (to == 0) { return ((TemporalGlobally<I>)subFml).getSubFml().toOwlString(); }
            else if (from == 0) { return "(" + ((TemporalGlobally<I>)subFml).getSubFml().toOwlString() + ") & (" +   new TemporalNext<I>(new TemporalSub<I>(subFml, from, to - 1), true).toOwlString() + ")"; }
            else { return new TemporalNext<I>(new TemporalSub<I>(subFml, from - 1, to - 1), true).toOwlString();}
        }
    }
    ...
    public String toAbstractString(OutputEquivalence mapper) {
        ...
        for (int i = this.from; i <= this.to; i++) {
            String builder = "( " +
                    "X (".repeat(Math.max(0, i)) +
                    subFml.subFml.toAbstractString(mapper) +
                    " )".repeat(Math.max(0, i)) +
                    " )";
            subFmls.add(builder);
        }
    ...
    public TemporalLogic<I> derivativeOn(String a){
        if (subFml instanceof TemporalEventually) {
            if (to == 0) { return ((TemporalEventually<I>)subFml).getSubFml().derivativeOn(a); }
            else if (from == 0) { return new TemporalOr<I>(((TemporalEventually<I>)subFml).getSubFml().derivativeOn(a), new TemporalSub<I>(subFml, from, to - 1)); }
            else { return new TemporalSub<I>(subFml, from - 1, to - 1); }
        } else {
            if (to == 0) { return ((TemporalGlobally<I>)subFml).getSubFml().derivativeOn(a);}
            else if (from == 0) { return new TemporalAnd<I>(((TemporalGlobally<I>)subFml).getSubFml().derivativeOn(a), new TemporalSub<I>(subFml, from, to - 1)); }
            else { return new TemporalSub<I>(subFml, from - 1, to - 1);}
        }  
    }

    public TemporalLogic<I> toNnf(boolean negate){
        return new TemporalSub<I>((TemporalOp<I>)subFml.toNnf(negate), from, to);
    }

    public TemporalLogic<I> toDisjunctiveForm(){
        return new TemporalSub<I>((TemporalOp<I>)subFml.toDisjunctiveForm(), from, to);
    }
