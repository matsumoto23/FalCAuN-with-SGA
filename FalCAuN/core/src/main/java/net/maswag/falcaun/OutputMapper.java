...
public class OutputMapper implements SULMapper<String, String, String, String>{
    @Getter
    private Map<String, String> outputMapper;
    private List<TemporalLogic.LTLFormula> formulaList;
    private List<Automaton<NormalformDELAConstruction.State,?>> automata;

    public OutputMapper(List<TemporalLogic.LTLFormula> formulaList, Alphabet<String> sigma, Alphabet<String> gamma, boolean partial){
        this.formulaList = formulaList;
        createDELAs(formulaList, sigma, gamma, partial);
        this.outputMapper = getOutputMapper(sigma, gamma);
    }
    ...
    // private Map<String, String> getOutputMapper(List<TemporalLogic.LTLFormula> formulaList, Alphabet<String> gamma){
    //     Map<String, String> mapper = new HashMap<>();
    //     for (int i = 0; i < gamma.size(); i++){
    //         String o1 = gamma.getSymbol(i);
    //         for (int j = 0; j < i; j++){
    //             String o2 = gamma.getSymbol(j);
    //             boolean equal = true; // check o1 is equivalent to o2
    //             for (TemporalLogic<String> formula : formulaList){ // for each phi in derivatives, check D_{o1}(phi) is equivalent to D_{o2}(phi)
    //                 TemporalLogic<String> derivative1 = formula.derivativeOn(o1);
    //                 TemporalLogic<String> derivative2 = formula.derivativeOn(o2);
    ...
    private Map<String, String> getOutputMapper(Alphabet<String> sigma, Alphabet<String> gamma){
        Map<String, String> mapper = new HashMap<>();
