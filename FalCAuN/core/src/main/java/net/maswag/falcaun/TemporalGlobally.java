...
public class TemporalGlobally<I> extends TemporalOp<I> {
    ...
    public String toAbstractString(OutputEquivalence mapper) {
        return String.format("[] ( %s )", subFml.toAbstractString(mapper));
    }
    ...
    public TemporalLogic<I> derivativeOn(String a){
        TemporalLogic<I> res = new TemporalAnd<I>( subFml.derivativeOn(a), this);
        return res;
    }
    
    public TemporalLogic<I> toNnf(boolean negate){
        if (negate){
            return new TemporalEventually<I>(subFml.toNnf(negate));
        } else {
            return new TemporalGlobally<I>(subFml.toNnf(negate));
        }
    }

    public TemporalLogic<I> toDisjunctiveForm(){
        return new TemporalGlobally<I>(subFml.toDisjunctiveForm());
    }
