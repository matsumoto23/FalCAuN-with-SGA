...
public abstract class AbstractTemporalLogic<I> implements TemporalLogic<I> {
    ...
    protected String makeAbstractStringWithAtomicStrings(OutputEquivalence mapper) {
        if (!this.isInitialized()) {
            throw new IllegalStateException("The formula is not initialized but the abstract string is requested.");
        }
        if (this.satisfyingAtomicPropositions == null) {
        ...
        if (mapper != null) {
            mappedSatisfyingAtomicPropositions = satisfyingAtomicPropositions.stream().map(s -> mapper.mapAbstractOutput(s)).collect(Collectors.toSet());
        } else {
    ...
    @Override
    public boolean semanticallyEquals(TemporalLogic<I> formula){
        String s1 = this.toOwlString();
        String s2 = formula.toOwlString();
        owl.ltl.Formula formula1 = LtlParser.parse(s1).formula();
        owl.ltl.Formula formula2 = LtlParser.parse(s2).formula();
        return LanguageAnalysis.isEqual(formula1, formula2);
    }
}
