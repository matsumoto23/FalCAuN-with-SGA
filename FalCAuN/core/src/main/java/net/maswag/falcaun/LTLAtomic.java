...
public class LTLAtomic extends AbstractTemporalLogic<String> implements TemporalLogic.LTLFormula {
    ...
    public TemporalLogic<String> derivativeOn(String a){
        if (inputString.isPresent() && inputString.get().equals(a) ||
                outputString.isPresent() && outputString.get().equals(a)){
            return new TemporalConst<String>(true);
        } else {
            return new TemporalConst<String>(false);
        }
    }
