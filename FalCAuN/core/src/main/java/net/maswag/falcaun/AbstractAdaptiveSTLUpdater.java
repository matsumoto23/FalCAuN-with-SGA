...
public abstract class AbstractAdaptiveSTLUpdater<I> implements AdaptiveSTLUpdater<I> {
    ...
    private OutputEquivalence mapper = null;
            ...
            if (mapper != null){
                propertyOracles.add(
                        new LoggingPropertyOracle.MealyLoggingPropertyOracle<>(
                                new MealyFinitePropertyOracle<>(stl.toLTLString(mapper), inclusionOracle, emptinessOracle, modelChecker)));
            } else {
                propertyOracles.add(
                        new LoggingPropertyOracle.MealyLoggingPropertyOracle<>(
        ...
        if (!initialized) {
            if (mapper != null){
                STLProperties.forEach(stl ->
                        propertyOracles.add(
                                new LoggingPropertyOracle.MealyLoggingPropertyOracle<>(
                                        new MealyFinitePropertyOracle<>(stl.toLTLString(mapper),
    ...
    public List<String> getLTLProperties() {
        if (this.mapper != null){
            return this.getSTLProperties().stream().map(s -> s.toLTLString(this.mapper)).collect(Collectors.toList());
        } else {
            return getSTLProperties().stream().map(TemporalLogic::toLTLString).collect(Collectors.toList());
        }
    }

    public void setMapper(OutputEquivalence mapper){
        this.mapper = mapper;
        return;
    }

    public OutputEquivalence getMapper(){
        return this.mapper;
    }
}
