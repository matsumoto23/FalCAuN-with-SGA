...
class TemporalAnd<I> extends AbstractTemporalLogic<I> {
    ...
    public String toAbstractString() {
        // TemporalAnd<String> tmp = new TemporalAnd<>(new TemporalConst<>(true), new TemporalConst<>(true));
    ...
    public TemporalLogic<I> derivativeOn(String a){
        TemporalLogic<I> res = new TemporalAnd<I>(subFormulas.stream().map(f -> f.derivativeOn(a)).collect(Collectors.toList()));
        return res;
    }

    public TemporalLogic<I> toNnf(boolean negate){
        if (negate){
            return new TemporalOr<I>(subFormulas.stream().map(f -> f.toNnf(negate)).collect(Collectors.toList()));
        } else {
            return new TemporalAnd<I>(subFormulas.stream().map(f -> f.toNnf(negate)).collect(Collectors.toList()));
        }
    }

    public List<TemporalLogic<I>> getSubFmls(){
        return subFormulas;
    }
        ...
        for (TemporalLogic<I> formula: convertedSubFmls){
            ...
            if (formula instanceof TemporalOr<?>){
                if (currentList.isEmpty()){
                    ...
                    for (TemporalLogic<I> currentFormula: currentList){
                        for (TemporalLogic<I> subSubFormula: ((TemporalOr<I>)formula).getSubFmls()){
                            newList.add(new TemporalAnd<I>(currentFormula, subSubFormula));
                        }
                    }
                }
            } else {
                if (currentList.isEmpty()){
                    ...
                    for (TemporalLogic<I> currentFormula: currentList){
                        newList.add(new TemporalAnd<I>(formula, currentFormula));
                    }
                }
            }
            currentList = newList;
        }
        return new TemporalOr<I>(currentList);
    }
