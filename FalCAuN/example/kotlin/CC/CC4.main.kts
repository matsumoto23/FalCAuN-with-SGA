...
val yDiffMaxValues = listOf(50.0, null)
val yDiffMinValues = listOf(9.0, null)
...
// val diffy2y1Max = "signal(5)"
// val diffy3y2Max = "signal(6)"
val diffy4y3Max = "signal(7)"
val diffy5y4Max = "signal(8)"
val diffy4y3Min = "signal(9)"
val diffy5y4Min = "signal(10)"

// Define the STL properties
val stlFactory = STLFactory()
val stlList =
    listOf(
        "(alw ($diffy4y3Max < 50.0))",
        "(alw ($diffy5y4Max < 50.0))",
        "(alw ($diffy4y3Min > 9.0))",
        "(alw ($diffy5y4Min > 9.0))",
...
SimulinkSUL(initScript, paramNames, signalStep, 10.0 * simulinkSimulationStep).use { sul ->
    ...
    for (i in 0 until experimentSize) {
        ...
        // Run the experiment
        var result = runExperiment(verifier, "CC", "CC10")
        results.add(result)
    }
    FileOutputStream("result-CC10.csv").apply { writeCsv(results) }
    logger.info("The results are written to result-CC10.csv")
}
