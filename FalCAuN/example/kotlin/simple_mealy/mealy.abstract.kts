...
// 2. output alphabet contains strings "p", "q", and "r"
val gamma = Alphabets.fromList(listOf("p", "q", "r"))
...
val target: CompactMealy<String, String> = AutomatonBuilders.newMealy<String, String>(sigma)
    ...
    .from("q1")
        ...
        .on("b").withOutput("q").to("q3")
    .from("q2")
        .on("a").withOutput("q").to("q3")
        .on("b").withOutput("q").to("q3")
    .from("q3")
        .on("a").withOutput("r").to("q3")
        .on("b").withOutput("r").to("q3")
    .create()
// @formatter:on
...
val outputMapper = OutputMapper(ltlList, sigma, gamma, false)  // create a mapper using DISJSENSE
val mappedSUL = MappedSUL(outputMapper, sul)  // create the mapped system
val oracle = SULOracle(mappedSUL)
properties.setMemOracle(oracle)

// Configure and run the verifier
val verifier = BlackBoxVerifier(oracle, mappedSUL, properties, sigma)
...
// val learnedMealy = verifier.learnedMealy
