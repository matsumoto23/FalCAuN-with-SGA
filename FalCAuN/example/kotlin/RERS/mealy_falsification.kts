...
// Read the target Mealy machine from a dotfile
val file_index = args[1]
var wrapper = DotMealyWrapper("dotfiles/m" + file_index)
// 1. create input alphabet
val sigma = wrapper.sigma
// 2. create output alphabet
val gamma = wrapper.gamma

// Read LTL properties from a text file
...
} else if (args[0] == "partial") {   // DisjSecse
    mapper = OutputMapper(ltlList, sigma, gamma, true).outputMapper
} else if (args[0] == "abstract") {  // Coarsest
    mapper = OutputMapper(ltlList, sigma, gamma, false).outputMapper
}

// create a system mapped by abstraction.
...
println("# of states: ${verifier.getlearnedMealySize()}")
